...
import {notificationTime} from '../../../assets/data/notificationTime';
...
import createUserDoc from '../service/createUserDoc';

const OnBoardNotify = () => {
  const dispatch = useDispatch();
  const {onboarding} = useSelector(state => state.onBoardingSlice);
  const {auth} = useSelector(state => state.authSlice);

  const user = {
    ...auth,
    currencyDetails: JSON.parse(onboarding.currencyPicked),
  };
  ...
  const enableNotifications = async () => {
    const isEnabled = await PushNotifications(OnBoardNotifyRef, user);
    if (isEnabled) {
      dispatch(updateNotificationsEnabled(true));
      dispatch(onBoardCompletd(true));
    }
  };

  const onSkip = async () => {
    await createUserDoc(user);
    dispatch(updateNotificationsEnabled(false));
    dispatch(onBoardCompletd(true));
  };
    ...
    <View className="p-5 flex-1 justify-between">
      ...
      <View className="p-1 gap-4">
        ...
        <TouchableOpacity
          ...
          <Text className=" font-medium text-gray-500">I will do it later</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
